...
peg_path = os.path.join(os.path.dirname(__file__), 'grammars', 'calc_lr.peg')
json_path = os.path.join(os.path.dirname(__file__), 'grammars', 'calc_lr.json')

with open(json_path) as json_fd:
	grammar = json.load(json_fd)


class TestCalcLR(util.PeggyTestCase):
